...
export class AppTabsComponent implements OnInit {
  ...
  constructor(private datasharingService: DatasharingService) {
    this.subscription = this.datasharingService.salesHubTabs.subscribe((data) => {
  ...
  closeTab(tab: Tabs) {
    ...
    this.allSubscribedTabs = this.allSubscribedTabs.filter(stab => stab.id != tab.id);
  ...
  makeActive(tab: Tabs) {
    ...
    this.allSubscribedTabs.find(stab =>
      stab.id === tab.id
    ).active = true;
  }
}
