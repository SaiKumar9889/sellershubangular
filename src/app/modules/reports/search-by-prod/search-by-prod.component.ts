...
export class SearchByProdComponent implements OnInit {
  ...
  constructor(private appTrackingService:AppTrackingService,private downloadcsvService: DownloadcsvService,private toasterService: ToasterService, private searchByProductService: SearchByProductService) { 
    let ip:usertrackInput={menu:TrackMenus.REPORTS,submenu:subMenus.SALESBYPRODUCT,page:pages.SALESBYPRODUCTPAGE,function:"",descrption:"Sales By Product page loaded"};
  ...
  getReposts() {
    this.loading =true;
    this.searchByProductService.getReportByProduct(this.page, this.collectionsize, this.searchValue, this.searchBy, this.searchFrom, this.searchTo).subscribe((res: any) => {
      ////console.log(res);
  ...
  download() {
    this.loading=true;
    this.searchByProductService.getDownloadReport(this.searchFrom, this.searchTo,this.searchValue, this.searchBy).subscribe((res: any) => {
      ////console.log(res);
      if(res!=null)
