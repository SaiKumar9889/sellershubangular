...
export class StockValueRepoComponent implements OnInit {
  ...
  fromDate: any = '';
  constructor(private appTrackingService:AppTrackingService,private toasterService: ToasterService,private downloadcsvService: DownloadcsvService,private stockValueReportService: StockValueReportService) {
    let ip:usertrackInput={menu:TrackMenus.REPORTS,submenu:subMenus.STOCKVALUEREPORT,page:pages.STOCKVALUEREPORTPAGE,function:"",descrption:"Stock value report page loaded"};
  ...
  getReposts() {
    ...
    this.stockValueReportService.getStockValueReport(this.page, this.collectionsize,this.pageSize,this.search).subscribe((res: any) => {
      ////console.log(res);
  ...
  download() {
    this.loading=true;
    this.stockValueReportService.getDownloadReport(this.search,this.fromDate).subscribe((res: any) => {
      if(res!=null)
