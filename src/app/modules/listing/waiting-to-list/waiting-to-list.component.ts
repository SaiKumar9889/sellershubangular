...
import { WaitingToListService } from '../services/waiting-to-list.service';
declare var $: any;

@Component({
  selector: 'app-waiting-to-list',
  templateUrl: './waiting-to-list.component.html',
  styleUrls: ['./waiting-to-list.component.css']
})
export class WaitingToListComponent implements OnInit {
  channelProduct: any;
  productImages: any;
  channels: any;
  editChannel: boolean;
  ...
  constructor(private appTrackingService:AppTrackingService, private toasterService: ToasterService, private productService: ListingErrorsService,
    ...
    private channelproductService: ChannelProductsService, private listServ: WaitingToListService,private salsesOrdersService: SalsesOrdersService ) {
      let ip:usertrackInput={menu:TrackMenus.LISTINGS,submenu:subMenus.WAITINGTOLIST,page:pages.WAITINGTOLISTPAGE,function:"",descrption:"Waiting to list page loaded"};
  ...
  auditTrack(type:any){
    ...
    if(type==1)
    ip={menu:TrackMenus.LISTINGS,submenu:subMenus.WAITINGTOLIST,page:pages.WAITINGTOLISTPAGE,function:"",descrption:"Search button is clicked"};
    if(type==2)
    ip={menu:TrackMenus.LISTINGS,submenu:subMenus.WAITINGTOLIST,page:pages.WAITINGTOLISTPAGE,function:"",descrption:"Apply filter button is clicked"};
    if(type==3)
    ip={menu:TrackMenus.LISTINGS,submenu:subMenus.WAITINGTOLIST,page:pages.WAITINGTOLISTPAGE,function:"",descrption:"Reset button is clicked"};
    if(type==4)
    ip={menu:TrackMenus.LISTINGS,submenu:subMenus.WAITINGTOLIST,page:pages.WAITINGTOLISTPAGE,function:"",descrption:" Remove all Products button is clicked"};
    this.appTrackingService.trackUserActivity(ip);
  }
  modalclosed() {
  ...
  getProducts() {
    this.loading=true;
    this.listServ.getWaitingtolist(this.page,this.pageSize,this.total,this.searchName,this.searchValue,this.searchChannel).subscribe((res: any) => {
      ////console.log(res);
  ...
  onSubmit($event) {
    this.listServ.removewaitinglist().subscribe((res: any) => {
