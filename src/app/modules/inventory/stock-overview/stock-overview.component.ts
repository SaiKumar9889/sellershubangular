...
export class StockOverviewComponent implements OnInit {
  ...
  constructor(private appTrackingService:AppTrackingService, private toasterService: ToasterService, private stockService: StockOverviewService ) {
    let ip:usertrackInput={menu:TrackMenus.INVENTORY,submenu:subMenus.STOCKVIEW,page:pages.STOCKVIEWPAGE,function:"",descrption:"Stock view page loaded"};
  ...
  getStockOverview() {this.loading = true ;
    this.stockService.getStockview(this.page,this.collectionsize,this.pageSize, this.searchBy,this.searchValue,'','','','', false).subscribe((res: any) => {
      ////console.log(res);
  ...
  individualselection(event: any) {
    ...
    this.allStock.find(sale => sale.id == event.id).selected = !this.allStock.find(sale => sale.id == event.id).selected;
    let selectedItems = this.allStock.filter(sale => sale.selected == true);
    if (selectedItems.length > 0) {
      this.productIsSelected = true;
    } else {
  ...
  orderHistory(prod) {
    this.loading = true;
    this.stockService.viewOrderDetails(prod.sku).subscribe((res: any) => {
      //console.log(res);
