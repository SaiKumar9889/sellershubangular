...
export class CategoryComponent implements OnInit {
  ...
  subcategory: any[] = [];
  categoryDesc: any = '';
  constructor(private appTrackingService: AppTrackingService, private toasterService: ToasterService, private categoryService: CategoryService) {
  ...
  saveSelectedCate() {
    ...
    this.categoryService.saveCategory(ip).subscribe(res => {
      this.getCategories();
      this.toasterService.openToastMessage(ToastTypes.success, 'Category', 'Created successfully');
    });
  ...
  updateSelectedCate() {
    ...
    this.categoryService.saveCategory(ip).subscribe(res => {
      this.getCategories();
      this.toasterService.openToastMessage(ToastTypes.success, 'Category', 'Updated successfully');
    });
  }
  addSubcategory(){
    let ip = {
    ...
    this.categoryService.saveCategory(ip).subscribe(res => {
      this.getCategories();
      this.toasterService.openToastMessage(ToastTypes.success, 'Category', 'Subcategories added successfully');
    });
  }
  delete(data: any) {
    ...
    this.categoryService.delete(this.selectedCat.id).subscribe(res => {
      this.getCategories();
      this.toasterService.openToastMessage(ToastTypes.success, 'Category', 'Updated successfully');
    }, error => {
      if (error.error.message == "could not execute statement" || error.error.message == "Access denied") {
        this.toasterService.openToastMessage(ToastTypes.warning, 'Access denied', 'this category has been associate with the products');
      }
    });
  }
}
