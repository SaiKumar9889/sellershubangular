...
import { ShopeeInteService } from '../../services/shopee-inte.service';
...
@Component({
  ...
  styleUrls: ['./shopee.component.scss']
})
export class ShopeeComponent implements OnInit {
  ...
  autoSplit = false;
  constructor(private datasharingService: DatasharingService, private ShopeeInteService: ShopeeInteService, private toasterService: ToasterService, private shipService: ShippingRulesService, private splitserv: SpliRulesService, private channelintegrationService: ChannelIntegrationService) { }
  ...
  updateSpliRulz() {
    ...
    this.ShopeeInteService.updateIntegratedData(data).subscribe((res: any) => {
      console.log(res);
      this.loading = false;
    }, error => {
      console.log(error);
  ...
  getExistingDetails(channelId: any) {
    this.loading = true;
    this.ShopeeInteService.getAllreadyIntegratedData(channelId).subscribe((res: any) => {
      console.log(res);
  ...
  saveShopeeData() {
    if (this.name == '') {
      this.toasterService.openToastMessage(ToastTypes.warning, 'Shopee Integration', 'Please enter channel name');
      return;
    }
    // if (this.bonanzaid == '') {
    //   this.toasterService.openToastMessage(ToastTypes.warning, 'Shopee Integration', 'Please enter bonanza id');
    //   return;
    // }
    // if (this.clientid == '') {
    //   this.toasterService.openToastMessage(ToastTypes.warning, 'Shopee Integration', 'Please enter client id');
    //   return;
    // }
    // if (this.productType == '') {
    ...
    Swal.fire({
      ...
      ////console.log(result);
      if (result?.value) {
        this.ShopeeInteService.createIntegratedData(aData).subscribe((res: any) => {
          // console.log(res);
  ...
  updateShopeeData() {
    if (this.name == '') {
      this.toasterService.openToastMessage(ToastTypes.warning, 'Shopee Integration', 'Please enter name');
      return;
    }
    // if (this.bonanzaid == '') {
    //   this.toasterService.openToastMessage(ToastTypes.warning, 'Shopee Integration', 'Please enter bonanza id');
    ...
    Swal.fire({
      ...
      ////console.log(result);
      if (result?.value) {
        this.ShopeeInteService.updateIntegratedData(aData).subscribe((res: any) => {
          // console.log(res);
          this.channelId = res.id;
          this.isNewIntegration = false;
  ...
  testConnection() {
    Swal.fire({
      ...
      //////console.log(result);
      if (result?.value) {
        this.ShopeeInteService.testConnection(this.channelId).subscribe((res: any) => {
          //  console.log(res);
    ...
    this.ShopeeInteService.testConnection(this.channelId).subscribe((res: any) => {
      ...
      if (res == 'Channel Integration tested successfully') {
        Swal.fire({
          ...
          ////console.log(result);
          if (result?.value) {
            this.ShopeeInteService.downloadAllProducts(this.channelId).subscribe(res => {
              ////console.log(res)
              Swal.fire(
                'Shopee integration!',
  ...
  generateTocken() {
    Swal.fire({
      ...
      ////console.log(result);
      if (result?.value) {
        this.ShopeeInteService.generateTockent(this.channelId).subscribe((res: any) => {
          console.log(res);
          this.tocken = res.token;
          this.updateShopeeData();
        }, error => {
          ////console.log(error);
        });
        Swal.fire(
  ...
  deleteShopeeAccount() {
    Swal.fire({
      ...
      ////console.log(result);
      if (result?.value) {
        this.ShopeeInteService.deleteAmazonAccount(this.channelId).subscribe(res => {
          ////console.log(res)
        });
        Swal.fire(
          'Shopee integration!',
          'Shopee account is removed.',
          'success'
        )
      }
    });
