...
import { BonanzaInteService } from '../../services/bonanza-inte.service';
...
@Component({
  ...
  styleUrls: ['./bonanza.component.scss']
})
export class BonanzaComponent implements OnInit {
  ...
  autoSplit = false;
  constructor(private datasharingService: DatasharingService, private BonanzaInteService: BonanzaInteService, private toasterService: ToasterService, private shipService: ShippingRulesService, private splitserv: SpliRulesService, private channelintegrationService: ChannelIntegrationService) { }
  ...
  updateSpliRulz() {
    ...
    this.BonanzaInteService.updateIntegratedData(data).subscribe((res: any) => {
      console.log(res);
      this.loading = false;
    }, error => {
      console.log(error);
  ...
  getExistingDetails(channelId: any) {
    this.loading = true;
    this.BonanzaInteService.getAllreadyIntegratedData(channelId).subscribe((res: any) => {
      console.log(res);
      // this.apipassword = res?.apiKey;
      // this.url = res?.apiUrl;
      this.name = res?.name;
      this.bonanzaid = res?.merchantIdentifier;
  ...
  saveBonanzaData() {
    if (this.name == '') {
      this.toasterService.openToastMessage(ToastTypes.warning, 'Bonanza Integration', 'Please enter channel name');
      return;
    }
    if (this.bonanzaid == '') {
      this.toasterService.openToastMessage(ToastTypes.warning, 'Bonanza Integration', 'Please enter bonanza id');
      return;
    }
    // if (this.clientid == '') {
    //   this.toasterService.openToastMessage(ToastTypes.warning, 'Bonanza Integration', 'Please enter client id');
    //   return;
    // }
    // if (this.productType == '') {
    ...
    let aData = {
      "id": localStorage.getItem("userId"),
      "name": this.name,
      "merchantIdentifier": this.bonanzaid,
      "type": "bonanza",
      "downloadInventory": this.catLogIsChecked,
      "downloadOrders": this.ordersIsChecked,
      "maxListed": this.maxListed,
    ...
    Swal.fire({
      ...
      ////console.log(result);
      if (result?.value) {
        this.BonanzaInteService.createIntegratedData(aData).subscribe((res: any) => {
          // console.log(res);
  ...
  updateBonanzaData() {
    if (this.name == '') {
      this.toasterService.openToastMessage(ToastTypes.warning, 'Bonanza Integration', 'Please enter name');
      return;
    }
    if (this.bonanzaid == '') {
      this.toasterService.openToastMessage(ToastTypes.warning, 'Bonanza Integration', 'Please enter bonanza id');
    ...
    Swal.fire({
      ...
      ////console.log(result);
      if (result?.value) {
        this.BonanzaInteService.updateIntegratedData(aData).subscribe((res: any) => {
          // console.log(res);
          this.channelId = res.id;
          this.isNewIntegration = false;
  ...
  testConnection() {
    Swal.fire({
      ...
      //////console.log(result);
      if (result?.value) {
        this.BonanzaInteService.testConnection(this.channelId).subscribe((res: any) => {
          //  console.log(res);
    ...
    this.BonanzaInteService.testConnection(this.channelId).subscribe((res: any) => {
      ...
      if (res == 'Channel Integration tested successfully') {
        Swal.fire({
          ...
          ////console.log(result);
          if (result?.value) {
            this.BonanzaInteService.downloadAllProducts(this.channelId).subscribe(res => {
              ////console.log(res)
              Swal.fire(
                'Bonanza integration!',
  ...
  generateTocken() {
    Swal.fire({
      ...
      ////console.log(result);
      if (result?.value) {
        this.BonanzaInteService.generateTockent(this.channelId).subscribe((res: any) => {
          // console.log(res);
          if (res.token == null) {
            Swal.fire(
  ...
  deleteBonanzaAccount() {
    Swal.fire({
      ...
      ////console.log(result);
      if (result?.value) {
        this.BonanzaInteService.deleteAmazonAccount(this.channelId).subscribe(res => {
          ////console.log(res)
        });
        Swal.fire(
          'Bonanza integration!',
          'Bonanza account is removed.',
          'success'
        )
      }
    });
