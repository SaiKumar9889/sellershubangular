...
import { Mazento2Service } from '../services/mazento2.service';
...
@Component({
  selector: 'app-magento2-inte',
  templateUrl: './magento2-inte.component.html',
  styleUrls: ['./magento2-inte.component.css']
})
export class Magento2InteComponent implements OnInit {
  ...
  importToMagento2: boolean = false;
  maxListed: any;
  updateInventory: any;
  stockPercentage: any;
  priceChange: any;
  ...
  autoSplit = false;
  constructor(private datasharingService: DatasharingService, private mazento2Service: Mazento2Service, private toasterService: ToasterService, private shipService: ShippingRulesService, private splitserv: SpliRulesService, private channelintegrationService: ChannelIntegrationService) { }
  ...
  updateSpliRulz() {
    ...
    this.mazento2Service.updateIntegratedData(data).subscribe((res: any) => {
      console.log(res);
      this.loading = false;
    }, error => {
      console.log(error);
  ...
  getExistingDetails(channelId: any) {
    this.mazento2Service.getAllreadyIntegratedData(channelId).subscribe((res: any) => {
      ////console.log(res);
      this.name = res?.name;
      this.password = res?.apipassword;
      this.username = res?.apiusername;
      this.url = res?.apiUrl;
      this.imagepath = res?.fbareportStatus;
  ...
  saveMagento2Data() {
    if (this.name == '') {
      this.toasterService.openToastMessage(ToastTypes.warning, 'Magento2 Integration', 'Please enter name');
      return;
    }
    if (this.username == '') {
      this.toasterService.openToastMessage(ToastTypes.warning, 'Magento2 Integration', 'Please enter username');
      return;
    }
    if (this.password == '') {
      this.toasterService.openToastMessage(ToastTypes.warning, 'Magento2 Integration', 'Please enter password');
      return;
    }
    if (this.url == '') {
      this.toasterService.openToastMessage(ToastTypes.warning, 'Magento2 Integration', 'Please enter Magento2 url');
      return;
    }
    if (this.imagepath == '') {
      this.toasterService.openToastMessage(ToastTypes.warning, 'Magento2 Integration', 'Please enter image path');
      return;
    }
    ...
    Swal.fire({
      ...
      ////console.log(result);
      if (result?.value) {
        this.mazento2Service.createIntegratedData(aData).subscribe(res => {
          ////console.log(res);
          if (res == "Channel Integration created successfully") {
            Swal.fire(
              'Magento2 integration!',
              'Magento2 details are saved.',
              'success'
            )
  ...
  updateMagento2Data() {
    ...
    Swal.fire({
      ...
      ////console.log(result);
      if (result?.value) {
        this.mazento2Service.updateIntegratedData(aData).subscribe(res => {
          ////console.log(res);
          if (res == "Channel Integration updated successfully") {
            Swal.fire(
              'Magento2 integration!',
              'Magento2 details are updated.',
              'success'
            )
          } else {
            Swal.fire(res);
  ...
  deleteMagento2Account() {
    Swal.fire({
      ...
      ////console.log(result);
      if (result?.value) {
        this.mazento2Service.deleteAmazonAccount(this.channelId).subscribe(res => {
          ////console.log(res)
        });
        Swal.fire(
          'Magento2 integration!',
          'Magento2 account is removed.',
          'success'
        )
      }
    });
  ...
  testConnection() {
    Swal.fire({
      ...
      //////console.log(result);
      if (result?.value) {
        this.mazento2Service.testConnection(this.channelId).subscribe((res: any) => {
          //  console.log(res);
    ...
    this.mazento2Service.testConnection(this.channelId).subscribe((res: any) => {
      ...
      if (res == 'Channel Integration tested successfully') {
        Swal.fire({
          ...
          ////console.log(result);
          if (result?.value) {
            this.mazento2Service.downloadAllProducts(this.channelId).subscribe(res => {
              ////console.log(res)
              Swal.fire(
                'Magento2 integration!',
