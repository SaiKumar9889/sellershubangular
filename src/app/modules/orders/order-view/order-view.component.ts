...
export class OrderViewComponent implements OnInit, OnDestroy {
  ...
  getOrdeDetails(orderid: any) {
   ...
   this.viewOrderService.editOrderInfo(orderid).subscribe((res: any) => {
      ...
      this.orderStatusd = res?.order?.orderStatus;
      ...
      this.customerPhone = res?.order?.buyerPhoneNumber;
  ...
  getAllChannels() {
    this.createOrdersService.getChannelRegistration().subscribe(res => {
      ...
      this.getOrdeDetails(this.activeOrderInfo[0].khubOrderId?this.activeOrderInfo[0].khubOrderId:this.activeOrderInfo[0].channelOrderId);
    }, error => {
      this.loading = false;
    });
  }
  ...
  getProducts(page){
    this.loading = true;
    this.productService.getProducts(page, this.collectionsize, this.pageSize, 'all', 0, '0', '', 'all', 'all', 'all_supplier', false, false, true, false).subscribe((res: any) => {
      ////console.log(res);
      this.AllProducts = res?.products;
      ...
      Swal.fire({
        ...
        if (result?.value) {
          ...
          for (let i = 0; i < this.addedProducts.length; i++) {
            console.log(this.addedProductsSnapChat);
             var exist= this.addedProductsSnapChat.find(p => p.id == this.addedProducts[i].id);
            if(exist!=null){
              this.saveProduct(this.addedProducts[i], {channelSales : this.order.sales },true,exist.id);
            } else {
              const prod = await this.addProduct(this.addedProducts[i].id);
              await this.saveProduct(this.addedProducts[i], prod, false,null);
            }
  ...
  closeOrder(){
    this.datasharingService.closeNewEditOrderTab('edit');
  }
