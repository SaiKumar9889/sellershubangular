...
export class SalsesOrdersService {
  ...
  getPrinters() {
    let authToken = localStorage.getItem("userToken") as string as string;
  ...
  sendEmailBulk(orderid: any, attachLabel: any, dontShowPrices: any, includeOrderConfirmationCallbackUrl: any, invoiceOnly: any) {
    let authToken = localStorage.getItem("userToken") as string;
    let qparam = '?';
    orderid.forEach((ordid:any, index:any) => {
      if (index == 0)
  ...
  savePrintLabelsData(body:any, courier: any) {
    let authToken = localStorage.getItem("userToken") as string;
  ...
  async savePrintLabelsData1(body:any, courier: any) {
    let authToken = localStorage.getItem("userToken") as string;
...
async getPrintedLabelsData1(id:any){
  ...
  getEligibleServicesForAmazonBuyShipping(body:any) {
    let authToken = localStorage.getItem("userToken") as string;
    return this.http.post<any[]>(
