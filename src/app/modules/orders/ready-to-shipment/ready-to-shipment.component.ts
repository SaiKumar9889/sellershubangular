...
@Component({
  selector: 'app-ready-to-shipment',
  templateUrl: './ready-to-shipment.component.html',
  styleUrls: ['./ready-to-shipment.component.scss']
})
export class ReadyToShipmentComponent implements OnInit {
  loading: boolean = true; display: boolean = true;
  allSales: any[] = [];
  page = 0;
  ...
  constructor(private packingslipGenartionService: PackingslipGenartionService, private appTrackingService: AppTrackingService, private dbService: NgxIndexedDBService, private router: Router, private downloadcsvService: DownloadcsvService, private toasterService: ToasterService,
    private salsesOrdersService: SalsesOrdersService, private datasharingService: DatasharingService,
    @Inject(LOCALE_ID) public locale: string, public http: HttpClient, public datepipe: DatePipe, private pdftoblobService: PdftoblobService) {
  ...
  getDefaultData(pagenum: number = 1) {
    ...
    this.salsesOrdersService.getSalesOrders(pagenum, this.collectionsize, this.pageSize, this.selectedChannel, this.selectedFilter, this.searchname, this.searchValue2, '000', null, true, this.fliter, '000', 0).subscribe((sales_orders: any) => {
      // ////console.log(sales_orders);
      this.allSales = sales_orders?.sales || [];
      this.unShipped = this.allSales.filter(s => s.orderStatus == 'Unshipped');
  ...
  updatePrintedStatus(sale: any) {
    this.salsesOrdersService.updateLableUpdatedSTatus(sale.channelOrderId).subscribe(res => {
  ...
  openDownloadOrders() {
    this.loading = true;
    $('#ps-download-orders').modal('show');
    this.getChannelsForOrders();

  }
  modalDownloadOrdersclosed() {
    $('#ps-download-orders').modal('hide');
  }
  ...
  modalDispatchclosed() {
    $('#ps-dispatch-orders').modal('hide');
    this.searchDetails = false;
  }
  openlDispatchOrders() {
    $('#ps-dispatch-orders').modal('show');
    // this.getChannelsForOrders();
  }
  dispatch_order_id = '';
  batch_order_id = '';
  ...
  isPrintLabelSelectAll: boolean = false;
    ...
    for (let index = 0; index < this.selectedOrderData_res.length; index++) {
      ...
      for (let i = 0; i < order.channelSales.length; i++) {
        ...
        if (sale) {
          ...
          let img = await (await fetch(co?.imageUrl)).blob();
  ...
  cancelLabel() {
    ...
    this.salsesOrdersService.cancelLabel(shipmentId).subscribe(shipmentId => {
      ...
      if (error?.error?.text == "Shipment Cancelled Successfully..") {
        this.salsesOrdersService.afterCancelLabel(ids).subscribe(res => {
          console.log(res);
        }, error => {
          console.log(error);
          this.loading = false;
  ...
  exportOrders: any = []; exportCarrier: any = [];
  ...
  splitOrder() {
    $('#ps-split-order').modal('show');
    this.getOrderItems();
  }
  modalSplitClose() {
    $('#ps-split-order').modal('hide');
  ...
  viewOrderTab() {
    this.datasharingService.previousMenu = AllMenuTabs.ready_to_shipment;
  ...
  async appendData(page: PDFPage, fields: any, j: any, k: any, selectedItem, pdfDoc: PDFDocument) {
    ...
    for (let index = 0; index < fields.length; index++) {
      if (fields[index].key == '${product_image!}' && selectedItem.imageUrl) {
        ...
        if (selectedItem.imageUrl && selectedItem.imageUrl.includes('https://s3-us-west-2.amazonaws.com/synmultichannelprod/')) {
          pngImageBytes = await this.pdftoblobService.s3Image(selectedItem.imageUrl);
        } else {
          pngImageBytes = await fetch(selectedItem.imageUrl, { mode: 'cors' }).then((res) => res.arrayBuffer(), (reason) => {
            console.log(reason);
            return null;
          });
        }
  ...
  selectCarrier: any = ''; track: any; shippingmethod: any = ''
  mapData1() {
      ...
      this.labeledOrders.forEach(order => {
        ...
        order.channelSales.forEach(cs => {
          ...
          if (cs?.labelPdfs?.length > 0) {
            ...
            this.openPdf(cs?.labelPdfs);
            this.updatePrintedStatus(cs);
          }
        });
        order.channelSales.forEach(cs => {
  ...
  printGenLaels() {
    ...
    this.salsesOrdersService.savePrintLabelsData(Obj, this.selectedCarrier_code).subscribe((res: any) => {
      ...
      if (res?.errororders?.length > 0) {
        ...
        this.toasterService.openToastMessage(ToastTypes.warning, 'Generate labels', res?.responcestatus);
