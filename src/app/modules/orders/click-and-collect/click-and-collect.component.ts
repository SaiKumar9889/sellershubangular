...
export class ClickAndCollectComponent implements OnInit {
  ...
  getChannelRegistration() {
    this.salsesOrdersService.getChannelRegistration().subscribe(res => {
      // console.log(res);
      this.channels = res;
      this.channels = this.channels.filter(s=>s.type == "amazon")
  ...
  getDefaultData(pagenum: number = 1) {
    ...
    // this.channelType.forEach(sale => this.selectedChannel.push(6956));
    this.selectedChannel.push(6956)
    console.log(this.selectedChannel)
    this.salsesOrdersService.getAllOrders().subscribe((sales_orders: any) => {
      console.log(sales_orders);
      // this.allSales = sales_orders?.sales || [];
      // this.allSales = this.allSales.filter(s=>s.channel == "amazon")
  ...
  cancelLabel() {
    ...
    this.salsesOrdersService.cancelLabel(shipmentId).subscribe(response => {
      ...
      selectedItems.forEach(element => {
        this.salsesOrdersService.afterCancelLabel(element.channelOrderId).subscribe(i => {
  ...
  shipStatus: any="ReadyForPickup";
  onSelectShippingStatus(e){
    this.loading = true;
    let ids:any;
     ids = e.khubOrderId?e.khubOrderId:e.channelOrderId;
     console.log(ids)
    let aData = {
      "orderId":ids,
	"orderShipmentStatus": this.shipStatus
    }
    this.salsesOrdersService.updateShipStatus(aData).subscribe(res => {
      console.log(res)
      this.loading = false;
    }, error => {
      this.loading = false;
